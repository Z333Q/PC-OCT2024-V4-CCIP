...
import "@chainlink/contracts/src/v0.8/ccip/CCIPClient.sol";
...
import "https://github.com/0xPolygonHermez/zkevm-contracts/blob/main/contracts/zkEVM/PolygonZkEVM.sol";
...
function fulfill(bytes32 _requestId, uint256 _teamId, uint256 _gameId, uint256 _score) public onlyOracle recordChainlinkFulfillment(_requestId) {
    ...
    // Account Abstraction Logic (EIP-4337)
    function executeTransaction(address to, uint256 value, bytes memory data) public returns (bool success) {
        // Ensure that only the wallet owner can initiate this
        require(msg.sender == owner(), "Only owner can execute transaction");
